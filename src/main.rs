...
impl Enemy {
    pub fn new(enemies: &mut Vec<Enemy>) -> Self {
        let x = get_new_random_position(enemies, 1, X_BORDER - 3);
        let y = get_new_random_position(enemies, 1, Y_BORDER - 3);
        Enemy { alive: true, x, y }
    }
...
fn get_new_random_position(enemies: &Vec<Enemy>, a: u16, b: u16) -> u16 {
    let mut rng = rand::rng();

    let number = rng.random_range(a..b);

    // give a position that would not collide with other enemies.
    for i in 0..enemies.len() {
        if number == enemies[i].x && number == enemies[i].y {
            return get_new_random_position(enemies, a, b);
        }
    }
    number
}
